...
async function handler(request) {
  ...
  const botToken =
    process.env.XY_DISCORD_BOT_TOKEN ||
  ...
  if (!guildId) {
    return json(
      {
        ok: false,
        message: "伺服器缺少 XY_GUILD_ID 或 NEXT_PUBLIC_XY_GUILD_ID。",
      },
      500
    );
  }
    ...
    return json(
      {
        ok: false,
        message:
          "伺服器缺少 XY_DISCORD_BOT_TOKEN 或 DISCORD_BOT_TOKEN，無法檢查 Discord 身分組。",
      },
      500
    );
  }
  ...
  if (!staffRoleIds.length) {
    return json(
      {
        ok: false,
        message: "伺服器缺少員工身分組設定，請設定 XY_STAFF_ROLE_IDS。",
      },
      500
    );
  }
  ...
  if (memberRes.status === 404) {
    return json(
      {
        ok: false,
        message: "你不在 XY陪玩 Discord 群組內，無法登入員工端。",
    ...
    return json(
      {
        ok: false,
        message:
          "Discord 身分組檢查失敗，請確認 XY Bot Token、群組 ID 是否正確。",
  ...
  const avatarUrl = getDiscordAvatarUrl(discordUser) || body?.avatar_url || null;
